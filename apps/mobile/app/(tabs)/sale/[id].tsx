...
export default function SaleOrderDetailScreen() {
  ...
  const { getOrder, auditOrder, cancelOrder } = useSaleOrders();
  ...
  const handleStockOut = () => {
    if (!order) return;
    router.push(`/sale/${order.id}/stock-out`);
  };

  const handleReturn = () => {
    if (!order) return;
    router.push(`/sale/${order.id}/return`);
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Surface style={styles.section} elevation={1}>
        ...
        <ListItem
          title={order.customer?.name || '未知客户'}
          subtitle={order.customer?.code}
          left={<Icon source="account" size={24} color="#2196f3" />}
        />
        <Divider />
        <ListItem
          title="联系电话"
          subtitle={order.phone || order.customer?.mobile || '-'}
      ...
      <Surface style={styles.section} elevation={1}>
        ...
        <ListItem
          ...
          left={<Icon source="warehouse" size={24} color="#ff9800" />}
        />
      </Surface>

...
import { purchaseApi, PurchaseReturn } from '@/api/purchase';
...
const PurchaseReturnList = () => {
  ...
  const [dataSource, setDataSource] = useState<PurchaseReturn[]>([]);
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      const res = await purchaseApi.listReturns({ ...query, tenantId } as any);
  ...
  const handleAudit = async (id: string) => {
    await purchaseApi.auditReturn(id);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await purchaseApi.createReturn(data as any);
  ...
  const columns = [
    ...
    { title: '退货日期', dataIndex: 'stockInDate', key: 'stockInDate', width: 100, render: (d: string) => dayjs(d).format('YYYY-MM-DD') },
  ...
  return (
    <>
      <Card>
        ...
        <Table title={() => <Button type="primary" icon={<PlusOutlined />} onClick={handleAdd}>新增采购退货</Button>}
      ...
      <Modal title="新增采购退货" open={modalVisible} onCancel={() => setModalVisible(false)}
        ...
        <Form form={form} layout="vertical">
          <Space style={{ display: 'flex' }} align="start" size="large">
            ...
            <Form.Item name="returnDate" label="退货日期" rules={[{ required: true }]} style={{ width: 150 }}>
              <DatePicker />
            </Form.Item>
          </Space>
          ...
          <Table
            ...
            columns={[
              ...
              { title: '退货数量', width: 100, render: (_: any, __: any, i: number) => (
          ...
          <div style={{ textAlign: 'right', marginTop: 8, fontSize: 16 }}>
            合计: <span style={{ color: '#cf1322', fontWeight: 600 }}>-¥{totalAmount.toFixed(2)}</span>
          </div>
        </Form>
      </Modal>
    </>

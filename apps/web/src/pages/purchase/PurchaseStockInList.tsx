...
import { purchaseApi, PurchaseStockIn } from '@/api/purchase';
...
const PurchaseStockInList = () => {
  ...
  const [dataSource, setDataSource] = useState<PurchaseStockIn[]>([]);
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      const res = await purchaseApi.listStockIns({ ...query, tenantId } as any);
  ...
  const handleAudit = async (id: string) => {
    await purchaseApi.auditStockIn(id);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await purchaseApi.createStockIn(data as any);
  ...
  const columns = [
    ...
    { title: '日期', dataIndex: 'stockInDate', key: 'stockInDate', width: 100, render: (d: string) => dayjs(d).format('YYYY-MM-DD') },
    ...
    { title: '状态', dataIndex: 'status', key: 'status', width: 80, render: (s: string) => <Tag {...statusMap[s]}>{statusMap[s]?.label || s}</Tag> },
    {
      ...
      render: (_: any, r: PurchaseStockIn) => (
        <Space size="small">
  ...
  return (
    <>
      <Card>
        ...
        <Table title={() => <Button type="primary" icon={<PlusOutlined />} onClick={handleAdd}>新增采购入库</Button>}
      ...
      <Modal title="新增采购入库" open={modalVisible} onCancel={() => setModalVisible(false)}
        ...
        <Form form={form} layout="vertical">
          <Space style={{ display: 'flex' }} align="start" size="large">
            ...
            <Form.Item name="stockInDate" label="入库日期" rules={[{ required: true }]} style={{ width: 150 }}>
              <DatePicker />
            </Form.Item>
          </Space>

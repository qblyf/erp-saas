...
import { saleApi, SaleReturn } from '@/api/sale';
...
const SaleReturnList = () => {
  ...
  const [dataSource, setDataSource] = useState<SaleReturn[]>([]);
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      const res = await saleApi.listReturns({ ...query, tenantId } as any);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await saleApi.createReturn(data as any);
  ...
  const columns = [
    ...
    { title: '退货日期', dataIndex: 'stockOutDate', key: 'stockOutDate', width: 100, render: (d: string) => dayjs(d).format('YYYY-MM-DD') },
  ...
  return (
    <>
      <Card>
        ...
        <Table title={() => <Button type="primary" icon={<PlusOutlined />} onClick={handleAdd}>新增销售退货</Button>}
      ...
      <Modal title="新增销售退货" open={modalVisible} onCancel={() => setModalVisible(false)}

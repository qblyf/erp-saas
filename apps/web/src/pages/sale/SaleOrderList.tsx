...
const SaleOrderList = () => {
  ...
  const [dataSource, setDataSource] = useState<SaleOrder[]>([]);
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      const res = await saleApi.listOrders({ ...query, tenantId } as any);
  ...
  const handleEdit = async (record: SaleOrder) => {
    setEditingId(record.id);
    fetchCustomers();
    fetchWarehouses();
    fetchProducts();
    const res = await saleApi.getOrder(record.id);
    form.setFieldsValue({
      ...res,
  ...
  const handleDelete = async (id: string) => {
    await saleApi.deleteOrder(id);
    message.success('删除成功');
    fetchData();
  };

  const handleAudit = async (id: string) => {
    await saleApi.auditOrder(id);
    message.success('审核成功');
    fetchData();
  };

  const handleCancel = async (id: string) => {
    await saleApi.cancelOrder(id);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingId) {
        await saleApi.updateOrder(editingId, data);
      } else {
        await saleApi.createOrder(data as any);
      }
  ...
  return (
    <>
      <Card>
        ...
        <Table title={() => <Button type="primary" icon={<PlusOutlined />} onClick={handleAdd}>新增销售订单</Button>}
      ...
      <Modal title={editingId ? '编辑销售订单' : '新增销售订单'} open={modalVisible} onCancel={() => setModalVisible(false)}
